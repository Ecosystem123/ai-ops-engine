...
try:
  s = requests.get('http://localhost:9835/metrics')
  print("local system gpu details are: \n")
  data = {}
  k = s.text
  for i in  text_string_to_metric_families(k):
   for j in i.samples:
     ...
     if j.name == 'nvidia_smi_power_draw_average_watts':
       print("nvidia_smi_power_draw_average_watts:",j.value)
       data["nvidia_smi_power_draw_average_watts"] = j.value
